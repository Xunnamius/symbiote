...
Provide --force to force this command to run post-npm-install scripts without regard for which (if any) npm scripts are running. In such a scenario, post-npm-install scripts should take advantage of the available SYMBIOTE_* environment variables to alter their functionality (e.g. only running certain tasks when SYMBIOTE_NPM_IS_INSTALLING=true).

However, if the runtime pre-checks fail, this command will always exit as a no-op regardless of the flags passed. Similarly, regardless of the presence of --force, Husky will NEVER execute in a CI or non-development environment.

Typically, this command should not be executed manually but by your package manager automatically at "install time," i.e. when running \`npm install\`/\`npm ci\`. With respect to NPM, this command should be run whenever NPM would run its "prepare" life cycle operation. Therefore, note that post-npm-install scripts MUST BE IDEMPOTENT and EXTREMELY LIGHTWEIGHT, as they WILL be invoked multiple times over the lifetime of long-lived projects, including at ODD TIMES like during package publication/release. See https://docs.npmjs.com/cli/v10/using-npm/scripts#life-cycle-operation-order for details.
